...
const MemoryUsageChart = ({
  ...
  const { fetchAsync, isFetching, error, data } = useGetApplicationMetric(appName, 'memory_usage');
  ...
  if (error) {
    return (
      <Alert className="mb-4" variant="destructive">
        ...
        <AlertTitle>{t('Error loading memory usage chart')}</AlertTitle>
  ...
  return (
    <div className="overflow-hidden rounded-md border flex flex-col gap-4 min-h-0 w-full h-full p-2">
      <h3 className="text-sm font-semibold">{t('Memory Usage (Bytes)')}</h3>
      <ChartContainer config={chartConfig} title={t('Memory Usage (Bytes)')} className="h-[200px] w-full">
