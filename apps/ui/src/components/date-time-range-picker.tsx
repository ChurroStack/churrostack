...
export const DateTimeRangePicker: React.FC<DateTimeRangePickerProps> = ({
  ...
  const [calendarMonths, setCalendarMonths] = React.useState<[Date, Date]>([new Date(), addMonths(new Date(), 1)]);
  ...
  //   if (!a || !b) return a === b;
  //   return isEqual(a.from || new Date(), b.from || new Date()) && isEqual(a.to || new Date(), b.to || new Date());
  // };
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={cn('w-full sm:w-[300px] justify-start text-left text-[11px] font-normal text-wrap', className)}>
          ...
          <div className="space-y-4 p-4">
            <div className="hidden lg:flex space-x-4">
              ...
              <Calendar
                mode="range"
                selected={range}
                onSelect={(newRange) => newRange && setRange(newRange as DateTimeRange)}
              ...
              <Calendar
                ...
                onMonthChange={(month) => setCalendarMonths([subMonths(month, 1), month])}
                className="border rounded-md"
              />
            </div>
