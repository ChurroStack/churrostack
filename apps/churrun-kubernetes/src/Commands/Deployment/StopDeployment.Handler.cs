...
{
    public class StopDeploymentHandler : IRequestHandler<StopDeployment, Task>
    {
        private readonly KubernetesService _kubernetesService;
        private readonly IConfiguration _configuration;

        public StopDeploymentHandler(KubernetesService kubernetesService, IConfiguration configuration)
        {
            _kubernetesService = kubernetesService;
            _configuration = configuration;
        }

        public async Task Handle(StopDeployment request, CancellationToken cancellationToken)
        {
            bool exists = false;
            ...
            {
                exists = true;
                await _kubernetesService.ScaleManifestAsync(deploymentName, @namespace, 0);
            }
            if (!exists)
            {
