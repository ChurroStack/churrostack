...
namespace ChurrOS.AppHost
{
    ...
    {
        internal static bool _useExternalAddres = false;

        public static IResourceBuilder<YarpResource> AddYarp(this IDistributedApplicationBuilder builder, string name, bool useExternalAddress = false)
        {
            _useExternalAddres = useExternalAddress;
        ...
        public static IResourceBuilder<YarpResource> Route(this IResourceBuilder<YarpResource> builder, string routeId, string target, Uri targetUri, string? path = null, string[]? hosts = null, bool preservePath = false, int? order = null)
        {
            builder.Resource.RouteConfigs[routeId] = new()
            {
                RouteId = routeId,
            ...
            if (builder.Resource.ClusterConfigs.ContainsKey(target))
            {
                return builder;
            }

            builder.Resource.ClusterConfigs[target] = new()
            {
                ClusterId = target,
                Destinations = new Dictionary<string, DestinationConfig>
                {
                    [target] = new() { Address = targetUri.ToString() }
                }
            };

            builder.WithReference(target, targetUri);
        ...
        ResourceLoggerService resourceLoggerService) : IDistributedApplicationLifecycleHook, IAsyncDisposable
    {
        ...
        public async Task AfterEndpointsAllocatedAsync(DistributedApplicationModel appModel, CancellationToken cancellationToken = default)
        {
            ...
            var externalIp = YarpExtensions._useExternalAddres ? "0.0.0.0" : "127.0.0.1";
            var externalName = YarpExtensions._useExternalAddres ? Dns.GetHostName() : "localhost";

            if (yarpResource.Endpoints.Count == 0)
            {
                _app.Urls.Add($"http://{externalIp}:0");
            }
            else
            {
                foreach (var ep in yarpResource.Endpoints)
                {
                    ...
                    if (ep.Port is null)
                    {
                        _app.Urls.Add($"{scheme}://{externalIp}:0");
                    }
                    else
                    {
                        _app.Urls.Add($"{scheme}://{externalName}:{ep.Port}");
                    }
                }
            }
