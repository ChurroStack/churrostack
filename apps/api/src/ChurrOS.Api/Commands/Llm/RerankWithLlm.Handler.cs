...
{
    public class RerankWithLlmHandler : IRequestHandler<RerankWithLlm, ValueTask<JsonElement>>
    {
        ...
        public async ValueTask<JsonElement> Handle(RerankWithLlm request, CancellationToken cancellationToken)
        {
            ...
            destinationUriBuilder.Path = $"{destinationUriBuilder.Path.TrimEnd('/')}/{request.Api}";
